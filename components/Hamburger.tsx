...
export default function Hamburger({ children }: HamburgerProps) {
	...
	return (
		<nav>
			...
			<div className={classNames('border-2 border-black border-opacity-60 dark:border dark:border-white dark:border-opacity-60 rounded-md transition-colors z-50 flex flex-col items-center w-4/5 text-2xl bg-white dark:bg-black px-4 py-6 shadow-lg absolute top-full right-3', { 'hidden': !open })}>
				{React.Children.map(navItems, item => (
					React.cloneElement(item, { onClick: handleClickOnNavItem })
				))}
			</div>
		</nav>
	)
