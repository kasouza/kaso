 ...
 * @param subDir A directory under /posts/
 * @param id Post ID
 * @returns A list of image sources (empty if the given post doesn't have carousel images)
 */
export function getPostCarouselImages(subDir: string, id: string): string[] {
    ...
    if (fs.existsSync(imagesPath)) {
        images = fs.readdirSync(imagesPath).map(filename => `/images/posts/${subDir}/${id}/carousel/${filename}`)
    }
